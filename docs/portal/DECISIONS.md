...
| P-08 | Alta guiada de cliente dentro de la app (`#/alta`, solo socio): firma externa → alta con evidencia → PM → SOW → formalización (crea proyecto) → hitos → usuarios → activación | Materializa el flujo de origen de 2.4/5.2; en producción es la «herramienta ARSEG de publicación acotada» (8.1), con las mismas reglas de dominio (`domain/gestion.ts`) |
| P-09 | Inicio diferenciado por rol: socio = relación estratégica/comercial y riesgo; PM = operación (hitos, respuestas por validar, cortes); cliente = su servicio | Cada rol tiene un seguimiento distinto (II.3); retroalimentación del dueño de negocio |
| P-10 | El detalle operativo del portal son los hitos y compromisos PUBLICADOS al cliente; las tareas internas del equipo ARSEG siguen excluidas (1.3) | Sustituye la coordinación con el cliente (el "Asana" de cara al cliente) sin convertir el portal en gestor de tareas internas; si se quisiera gestión interna, requiere revisión de la SPEC |
